...
def test_transport_is_pinned_revalidated_and_removed_with_a_lease() -> None:
    ...
    assert '--force-with-lease="$ref:$TRANSPORT_SHA"' in WORKFLOW
...
def test_old_abandoned_candidate_cannot_be_promoted() -> None:
    ...
    assert promotion.index("require_newer_semver.py") < promotion.index("gh release edit")


if __name__ == "__main__":
    test_only_trusted_default_branch_events_can_publish()
    test_transport_is_pinned_revalidated_and_removed_with_a_lease()
